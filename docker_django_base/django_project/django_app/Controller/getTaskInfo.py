...
class getTaskInfo:
    ...
    def get_task_record(self, request):
        ...
        inspect_list = sqlMethodClsss.select_workList(table_name="django_app_worklist w ", data_dic ={}, status_list ={"I"},column_list= column_list, option=option)
        print(inspect_list)
    ...
    def check_db_data(self, request, user_name, task_num):
        ...
        try:
            ...
            if condition_check.exists() :
                ...
                get_record = WorkRecord.objects.get(reg_id = user_name, work_category = "I", task_num = task_num, worklist_task_num_id = get_json_data)
                ...
                recombine_dict = {"id": get_id_list[0], "width":get_width_list[0], "height":get_height_list[0],
                                ...
                                "images":"["+"]", "length":get_length_list[0], "path":get_path_list[0], "type":get_type_list[0]}
            ...
            ## 재작업에 해당하므로 재작업 데이터 구성 로직 활성화
                ...
                get_record = WorkRecord.objects.get(reg_id = user_name, work_category = "T", task_num = task_num, worklist_task_num_id = get_json_data)
    ...
    def task_db_delete(self, request, user_name, task_num):
        ...
        endTime = parsing_data.get('end')
        print("---------------------------------------------------------------")
        print("Delete Region : start_time : " + str(startTime) + ", end_time : " + str(endTime) + ", reg_id : " + str(user_name) + ", work_id : " + str(task_num))
        print("---------------------------------------------------------------")
    ...
    def task_db_delete_abnormal(self, request, user_name, task_num):
        ...
        try:
            ...
            for task_id in TaskIdList:
                ...
                update_region = sql_query.update_status(table_name="django_app_tasklist", data_dic= update_dic, con_dic=con_dic)
            sql_query.close()
        except Exception as e:
            print("____________________________Delete__error!!!!!!!!____________________")
            print(e)
...
# 작업을 시작하면 task_num, user_name 해당하는 taskhistory 모두 select
    def task_db_select_all(self, user_name, task_num):
        try:
            ...
            instance = sql_query.select_workList(table_name="django_app_tasklist", data_dic=data_dic, column_list=column_list,option=option)
    ...
    def task_db_select(self, user_name, task_num):
        ...
        option = "order by start_time"
        try:
            task_data = sql_query.select_workList(table_name="django_app_tasklist", data_dic=data_dic, column_list=column_list, option=option)
        except:
            raise
        sql_query.close()
    ...
    def get_memo(self, task_num):
        try:
            ...
            memo = sql_query.select_workList(table_name="django_app_workhistory", data_dic=data_dic, column_list=column_list,option=option,status_list=None)
            sql_query.close()
