...
<<<<<<< .mine
        ...
        "(select code_nm  from django_app_code_mst where code_id = work_status)  work_status_nm,"+\
        "(select code_nm  from django_app_code_mst where code_id = work_type) work_type_nm "+\
        " from django_app_worklist where 1 = 1 and work_status in ('"+dbinfo.status['status_work_deagi']+"') "+\
...
>>>>>>> .r284
        ...
        #                 "(select code_nm  from django_app_code_mst where code_id = work_type) work_type_nm",
        #               ]
        # option = "and (select dap.group_id from django_app_profile dap where dap.account_id = '"+str(request.user)+"') = group_id and work_type = 'normal'   order by work_id limit 5"
        ...
        for item in task_list:
            ...
            if len(record_key) > 0:
                ...
                try:
                    ...
                    print("deny_check : ", deny_check, len(deny_check))
                    sql_query.close()
